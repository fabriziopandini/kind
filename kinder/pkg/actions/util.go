...
func waitForPodsRunning(kn *kcluster.KNode, label string, replicas int) error {
	for i := 0; i < 10; i++ {
		...
		//TODO: make the check manage more than one pod
		if lines, err := kn.CombinedOutputLines(
			"kubectl", "get", "pods", "-l", fmt.Sprintf("run=%s", label), "-o", "jsonpath='{.items[*].status.phase}'",
		); err == nil {
			if len(lines) != 1 {
...
func getNodePort(kn *kcluster.KNode, svc string) (string, error) {
	lines, err := kn.CombinedOutputLines(
		"kubectl", "get", "svc", svc, "--output=jsonpath='{range .spec.ports[0]}{.nodePort}'",
	)
	if err != nil {
...
func getPodName(kn *kcluster.KNode, label string) (string, error) {
	lines, err := kn.CombinedOutputLines(
		"kubectl", "get", "pods", "-l", fmt.Sprintf("run=%s", label), "-o", "jsonpath='{.items[0].metadata.name}'",
	)
	if err != nil {
