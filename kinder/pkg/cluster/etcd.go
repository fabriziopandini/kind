...
// this is temporary and should go away as soona s kind support external etcd node
func CreateExternalEtcd(name string) (ip string, err error) {
	// define name and labels mocking a kind external etcd node

	containerName := fmt.Sprintf("%s-%s", name, config.ExternalEtcdRole)
	...
	runArgs := []string{
		...
		"--label", fmt.Sprintf("%s=%s", constants.NodeRoleKey, config.ExternalEtcdRole),
	}

	// define a minimal etcd (insecure, single node, not exposed to the host machine)
	containerArgs := []string{
		"etcd",
		"--name", fmt.Sprintf("%s-ectd", name),
