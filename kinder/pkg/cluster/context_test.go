...
func newTestNode(name string, role config.NodeRole) *KNode {
...
var defaultNodes = KNodes{
	newTestNode("test-cp1", config.ControlPlaneRole),
}

var haNodes = KNodes{
	newTestNode("test-lb", config.ExternalLoadBalancerRole),
	newTestNode("test-cp1", config.ControlPlaneRole),
	newTestNode("test-cp2", config.ControlPlaneRole),
	newTestNode("test-cp3", config.ControlPlaneRole),
...
func TestResolveNodesPath(t *testing.T) {
	...
	cases := []struct {
		...
		{
			TestName:    "invalid-nodedelector path",
