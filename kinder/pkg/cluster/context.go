...
// is initialized by discoveryng the actual containers nodes
func NewKContext(ctx *cluster.Context) (c *KContext, err error) {
	...
	if c.BootStrapControlPlane() == nil {
		return nil, errors.Errorf("please add at least one node with role %q", config.ControlPlaneRole)
	}
...
func (c *KContext) CreateNode(role config.NodeRole, image string) error {
	...
	case config.ControlPlaneRole:
		...
		name := fmt.Sprintf("%s-%s%d", c.Name(), role, n)
		node, err := nodes.CreateControlPlaneNode(name, image, clusterLabel, nil)
		if err != nil {
...
// KubernetesNodes returns all the kuberentes nodes in the cluster
