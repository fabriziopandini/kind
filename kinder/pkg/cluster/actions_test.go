...
func TestExecutionPlanSorting(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			actual: executionPlan{
				&plannedTask{Node: newTestNode("worker2", config.WorkerRole)},
				&plannedTask{Node: newTestNode("control-plane2", config.ControlPlaneRole)},
				&plannedTask{Node: newTestNode("etcd", config.ExternalEtcdRole)},
				&plannedTask{Node: newTestNode("worker1", config.WorkerRole)},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole)},
			},
			expected: executionPlan{
				&plannedTask{Node: newTestNode("etcd", config.ExternalEtcdRole)},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole)},
		...
		},
		{
			...
			actual: executionPlan{
				&plannedTask{Node: newTestNode("worker1", config.WorkerRole), actionIndex: 3},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 2},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1},
				&plannedTask{Node: newTestNode("worker1", config.WorkerRole), actionIndex: 1},
			},
			expected: executionPlan{
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 2},
		...
		},
		{
			...
			actual: executionPlan{
				&plannedTask{Node: newTestNode("worker1", config.WorkerRole), actionIndex: 1, taskIndex: 2},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1, taskIndex: 2},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1, taskIndex: 1},
				&plannedTask{Node: newTestNode("worker1", config.WorkerRole), actionIndex: 1, taskIndex: 1},
			},
			expected: executionPlan{
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1, taskIndex: 1},
				&plannedTask{Node: newTestNode("control-plane1", config.ControlPlaneRole), actionIndex: 1, taskIndex: 2},
...
func TestNewExecutionPlan(t *testing.T) {
	testTopology := newTestCluster("test", KNodes{
		newTestNode("test-cp", config.ControlPlaneRole),
		newTestNode("test-w1", config.WorkerRole),
		newTestNode("test-w2", config.WorkerRole),
	})
